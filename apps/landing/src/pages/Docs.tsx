...
export default function Docs() {
  return <div><Navbar /><main className="docs-shell"><aside><p>GET STARTED</p>{sections.map(([id, label]) => <a key={id} href={`#${id}`}>{label}</a>)}<Link to="/console">Operator console →</Link></aside><article className="docs-content">
    ...
    <section id="release"><h2>How npm hosting works</h2><p>npm stores and distributes the compiled SDK packages. It does not run Dadieng’s API, site, indexer, replay workers, or Chainlink workflow.</p><ol><li>Remove <code>private: true</code> only from packages intended for release.</li><li>Set the scoped package access to public and add repository, license, files, and provenance metadata.</li><li>Build, test, create tarballs with <code>npm pack --dry-run</code>, then publish through GitHub Actions using npm trusted publishing.</li><li>Keep the unified website and console on web hosting, the control plane/workers on compute, and the indexer on Envio.</li></ol><Code>npm install @dadieng/sdk @dadieng/adapters</Code><p className="muted">This install command is the planned public channel; it must not be advertised as live until the registry release succeeds.</p></section>
