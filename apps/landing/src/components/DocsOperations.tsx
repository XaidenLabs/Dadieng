  ...
  return (
    <div className="docs-operations">
      ...
      <div className="ops-metrics" aria-label="Protocol metrics">
        <article><span>Threat receipts</span><strong>{data.metrics.receipts}</strong><small>sanitized incident records</small></article>
        <article><span>Protected events</span><strong>{data.metrics.protectedEvents.toLocaleString()}</strong><small>agent boundaries evaluated</small></article>
        <article><span>Validator agreement</span><strong>{percentage(data.metrics.validatorAgreement)}</strong><small>{data.metrics.validators} eligible identities</small></article>
        <article><span>P95 decision</span><strong>{data.metrics.p95LatencyMs === null ? '—' : `${data.metrics.p95LatencyMs} ms`}</strong><small>local enforcement path</small></article>
      </div>

      <div className="ops-grid">
        <section className="ops-card ops-defense">
          <div className="ops-card-head"><div><span className="eyebrow">Defense graph</span><h3>Active versions</h3></div><ShieldCheck /></div>
        ...
        <section className="ops-card">
          <div className="ops-card-head"><div><span className="eyebrow">Threat receipts</span><h3>Recent activity</h3></div><Activity /></div>
      ...
      <section className="ops-proof"><div><span className="eyebrow">Replay lab</span><h3>Canonical validation suite</h3><p>Attack cases, clean controls, decision drift, and evidence exposure are checked before a defense can be promoted.</p></div><div className="ops-proof-stats"><span><strong>20/20</strong>attacks blocked</span><span><strong>20/20</strong>controls preserved</span><span><strong>0</strong>decision drift</span><span><strong>0</strong>evidence exposed</span></div></section>
    </div>
  );
}
