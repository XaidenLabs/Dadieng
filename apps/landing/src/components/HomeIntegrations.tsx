...
type Connection = { name: string; short: string; role: string; status: 'connected' | 'ready' | 'degraded' | 'setup' };
...
const baseConnections: Connection[] = [
  ...
  { name: 'GitHub', short: 'GH', role: 'Remediation issue creation', status: 'setup' },
  { name: 'Slack', short: 'SL', role: 'Responder alerting', status: 'setup' },
  { name: 'Notion', short: 'NO', role: 'Incident knowledge base', status: 'setup' },
];
    ...
    Promise.allSettled([
      ...
      setConnections((current) => current.map((connection) => {
        ...
        if (readinessResult.status === 'fulfilled' && credentialKey in readinessResult.value.configured) {
          return { ...connection, status: readinessResult.value.configured[credentialKey] ? 'connected' : 'setup' };
        }
  ...
  return (
    <section id="integrations" className="section integrations home-connections">
      <div className="container">
        ...
        <div className="connection-grid">{connections.map((connection) => <article key={connection.name}><div className="connection-main"><span className="connection-mark">{connection.short}</span><div><h3>{connection.name}</h3><p>{connection.role}</p></div></div><div className={`connection-status ${connection.status}`}><i />{connection.status === 'setup' ? 'Connect account' : connection.status}</div></article>)}</div>
