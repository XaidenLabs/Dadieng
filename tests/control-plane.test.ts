...
describe("Dadieng control-plane API", () => {
  ...
  it("queues and completes a deterministic replay against the exact version", async () => {
    ...
    expect(resultBody.report.summary.attack).toEqual({ passed: 20, failed: 0, total: 20, passRate: 1 });
    expect(resultBody.report.summary.control).toEqual({ passed: 20, failed: 0, total: 20, passRate: 1 });
