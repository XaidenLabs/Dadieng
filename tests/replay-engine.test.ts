...
describe("deterministic replay engine", () => {
  it("passes the canonical 20-attack and 20-control suite", () => {
    const report = run();

    expect(report.cases).toHaveLength(40);
    expect(report.summary.attack).toEqual({ passed: 20, failed: 0, total: 20, passRate: 1 });
    expect(report.summary.control).toEqual({ passed: 20, failed: 0, total: 20, passRate: 1 });
