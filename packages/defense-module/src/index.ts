...
export function loadDefenseBundle(input: DefenseBundle): DefenseRule {
  ...
  return {
    ...
    evaluate(event) {
      const normalizedContent = event.content.toLocaleLowerCase("en-US");
      for (const rule of bundle.artifact.rules) {
        ...
        const matchesEveryGroup = rule.indicatorGroups.every((group) =>
          group.some((indicator) => normalizedContent.includes(indicator.toLocaleLowerCase("en-US"))),
        );
